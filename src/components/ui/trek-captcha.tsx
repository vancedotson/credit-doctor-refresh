...
interface TrekCaptchaProps {
  onVerify: (isVerified: boolean) => void;
  className?: string;
}

export const TrekCaptcha = ({ onVerify, className = '' }: TrekCaptchaProps) => {
  ...
  const loadCaptcha = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // For development, we'll extract the answer from the console
        // In production, this would be handled server-side
        setExpectedAnswer(''); // We'll need to handle this differently
      } else {
          ...
          <div className="flex-shrink-0">
            {isLoading ? (
              ...
              <img
                src={captchaImage}
                alt="Captcha"
                className="w-24 h-12 border rounded object-contain bg-white"
              />
            ) : (
